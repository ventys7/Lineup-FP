...
function renderFormation(){
  ...
  starterSlots["GK1"].style.position = "relative";
  starterSlots["GK1"].innerHTML = `<button class="gk-clear-btn" onclick="event.stopPropagation();clearStarterGoalkeeper()">✕</button>`;
  ...
  // Show bench container when team is loaded
  ...
  const benchSlots = {};
  const benchSlotIds = ["GK1", "GK2", "D1", "D2", "D3", "C1", "C2", "C3", "A1", "A2", "A3"];
  
  benchSlotIds.forEach(id => {
    benchSlots[id] = document.createElement("div");
    benchSlots[id].className = "slot empty";
    benchSlots[id].id = "slot-" + id;
    ...
    benchContainer.appendChild(benchSlots[id]);
  });
